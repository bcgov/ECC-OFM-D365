...
namespace OFM.Infrastructure.CustomWorkflowActivities.Application
{
    public sealed class SetFundingRecordStatus : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext executionContext)
        {
            ...
            try
            {
                ofm_application applicationRcord = (ofm_application)service.Retrieve(ofm_application.EntityLogicalName, recordId, new ColumnSet("statuscode", "ofm_application_type"));
                ...
                if (applicationRcord != null && applicationRcord.Attributes.Count > 0 && statusReason == (int)ofm_application_StatusCode.Verified)
                {
                    ...
                    //New Application: Recalculate Start Date and End Date for FA, rule: Adding two month (Verified anytime between January 1-31, the funding details record start date will be March 1.)
                    if (fundingRecords.Entities.Count > 0 && fundingRecords[0] != null)
                    {
                        ...
                        var applicationType = applicationRcord.GetAttributeValue<OptionSetValue>("ofm_application_type").Value;

                        if (applicationType == (int)ecc_application_type.New)
                        {
                            ...
                            entityToUpdate["ofm_start_date"] = calculatedStartDate;
                            entityToUpdate["ofm_end_date"] = calculatedEndDate;
                        }

                        service.Update(entityToUpdate);

                        tracingService.Trace("\nChange sucessfully active funding detail record status from Draft to FA review and Recalculate start date for new application");
                    }
                    else
                    {
