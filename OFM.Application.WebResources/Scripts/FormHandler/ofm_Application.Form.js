...
OFM.Application.Form = {
    ...
    //function to lock fields on editable grid
    lockFieldsOnLicenceDetailsGrid: function (context) {
        debugger;
        var formContext = window.pageContext;
        var gridContext = formContext.getControl("Subgrid_new_2");
        var selectedRow = gridContext.getGrid().getSelectedRows();
        //var licenceType = selectedRow.getAll()[0].getData().getEntity()._attributes.getByName("ofm_care_type")
        //selectedRow.getAll()[0].getData().entity.attributes.getByName("ofm_care_type").controls.getByName("ofm_care_type").setDisabled(true);
        //selectedRow.getAll()[0].getData().entity.attributes.forEach(function (attr) {
        //    if (attr.getName() === "ofm_care_type") {
    ...
    //checkLicenceType: function (formContext,licenceId) {
    //    var removeValues;
