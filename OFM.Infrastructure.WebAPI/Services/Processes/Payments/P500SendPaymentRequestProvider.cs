...
public class P500SendPaymentRequestProvider(IOptionsSnapshot<ExternalServices> bccasApiSettings, ID365AppUserService appUserService, ID365WebApiService d365WebApiService, ILoggerFactory loggerFactory, TimeProvider timeProvider) : ID365ProcessProvider
{
    ...
    public string RequestPaymentLineUri
    {
        get
        {
            var LocalDateOnly_Today = DateTime.Now.Date;
            var fetchXml = $"""
                    <fetch version="1.0" output-format="xml-platform" mapping="logical" distinct="false">
                      <entity name="ofm_payment">
                       ...
                       <attribute name="ofm_invoice_date" />
                        ...
                        <filter type="or">
                          <condition attribute="ofm_invoice_date" operator="eq" value="{LocalDateOnly_Today}" />
                          <condition attribute="ofm_revised_invoice_date" operator="eq" value="{LocalDateOnly_Today}" />
                        </filter>
                          </filter>
