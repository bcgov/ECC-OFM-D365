...
public class FundingCalculator : IFundingCalculator
{
    ...
    private IEnumerable<LicenceDetail> LicenceDetails
    {
        get
        {
            var licenceDetails = _funding?.ofm_facility?.ofm_facility_licence?.Where(lic => lic.statuscode == ofm_licence_StatusCode.Active).SelectMany(licence => licence?.ofm_licence_licencedetail!).Where(licenceDetail => licenceDetail.statuscode == ofm_licence_detail_StatusCode.Active);
