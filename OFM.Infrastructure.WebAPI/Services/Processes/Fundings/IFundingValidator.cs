...
public class MustHaveValidLicenceRule : IFundingValidator<Funding>
{
    ...
    public bool Validate(Funding funding)
    {
        ...
        licence.ofm_start_date <= (funding.ofm_application!.ofm_summary_submittedon ?? funding.ofm_application!.createdon ?? new DateTime()) &&
        (licence.ofm_end_date is null || licence.ofm_end_date >= (funding.ofm_application!.ofm_summary_submittedon ?? funding.ofm_application!.createdon ?? new DateTime())))?.Count();
