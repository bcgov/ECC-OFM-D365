...
namespace OFM.Infrastructure.Plugins
{
    ...
    public class LocalPluginContext : ILocalPluginContext
    {
        ...
        private Entity GetTarget()
        {
            switch (PluginExecutionContext.MessageName)
            {
                ...
                case "Delete":
                    ...
                    var entity = InitiatingUserService.Retrieve(entityRef.LogicalName, entityRef.Id, new Microsoft.Xrm.Sdk.Query.ColumnSet(true));
                    return entity;
                default:
        ...
        {
            var image = (PluginExecutionContext.Stage == 20 && PluginExecutionContext.PreEntityImages.ContainsKey("Image")) ?
                PluginExecutionContext.PreEntityImages["Image"] : 
                (PluginExecutionContext.Stage == 40 && PluginExecutionContext.PostEntityImages.ContainsKey("Image")) ?
                PluginExecutionContext.PostEntityImages["Image"] :
                (PluginExecutionContext.MessageName == "Create") ? 
                (Entity)PluginExecutionContext.InputParameters["Target"] :
                InitiatingUserService.Retrieve(Target.LogicalName, Target.Id, new Microsoft.Xrm.Sdk.Query.ColumnSet(true));
