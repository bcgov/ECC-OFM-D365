            ...
            if (localPluginContext.Target.Contains(ofm_application.Fields.statuscode))
            {
                using (var crmContext = new DataverseContext(localPluginContext.PluginUserService))
                {
                    ...
                    if (status.Value == (int) ofm_application_StatusCode.Submitted)
                    {
                        ...
                        if (facility != null)
                        {
                            var programStartDate = localPluginContext.PluginUserService.Retrieve(facility.LogicalName, facility.Id, new ColumnSet(true)).GetAttributeValue<DateTime>(Account.Fields.ofm_program_start_date);
                            localPluginContext.Trace($"programStartDate {programStartDate}");
                            ...
                            if (applicationSubmittedOn >= programStartDate.AddYears(1))
                            {
                                //Set CCOF Participation flag to true
                                entity["ofm_system_ccof_tdad_participation"] = true;
                            ...
                            {
                                //Set CCOF Participation flag to false
                                entity["ofm_system_ccof_tdad_participation"] = false;
                                UpdateRequest updateRequest = new UpdateRequest { Target = entity };
                                crmContext.Execute(updateRequest);
                            }
                        }
